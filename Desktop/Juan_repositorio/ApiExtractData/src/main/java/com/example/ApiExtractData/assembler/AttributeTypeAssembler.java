...
public class AttributeTypeAssembler {
    ...
    public AttributeType attributeTypeToEntity(AttributeTypeDTO attributeTypeDto) {
        ...
        if (attributeTypeDto.getId() != null) {
            Optional<AttributeType> existingAttributeType = attributeTypeRepository.findById(attributeTypeDto.getId());// buscamos un attributeType por su id
            if (existingAttributeType.isPresent()) {
                return existingAttributeType.get(); // Retornar el AttributeType existente
            }
        }
