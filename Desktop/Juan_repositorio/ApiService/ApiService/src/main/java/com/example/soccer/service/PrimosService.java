...
public class PrimosService{
    ...
    public ArrayList<Integer> encontrarPrimosConSumandos(int n) {
        ...
        for (int primo : primos) {//itera sobre el ArrayList de primos
            int longitud = encontrarCadenaMasLarga(primo);//variable para guardar la cadena mas larga de sumandos primos consecutivos que suman el numero primo (en el contexto de este primo que estamos iterando).
    ...
    private boolean esPrimo(int n) {
        ...
        for (int i = 3; i * i <= n; i += 2) {// iteramos desde el 3 de 2 en 2 hasta que i valga la mitad de n
            if (n % i == 0) {
